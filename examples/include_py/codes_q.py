...
from scipy.sparse import identity, hstack, kron, csr_matrix # type: ignore
from include_py.utils import row_echelon, rank, kernel, compute_code_distance, inverse, int2bin
...
def create_margulis_code(hx,hz,a1,a2,a3,b1,b2,b3,Name=None):
    sparse_matrix1 = csr_matrix(a1)
    sparse_matrix2 = csr_matrix(a2)
    sparse_matrix3 = csr_matrix(a3)
    A_list = [sparse_matrix1, sparse_matrix2, sparse_matrix3]
    sparse_matrix1 = csr_matrix(b1)
    sparse_matrix2 = csr_matrix(b2)
    sparse_matrix3 = csr_matrix(b3)
    B_list = [sparse_matrix1, sparse_matrix2, sparse_matrix3]
    return  css_code(hx,hz,name=Name), A_list, B_list



def create_bicycle_codes(n, A_pows, B_pows, name=None):
    """
    Construct a (single-variable) bicycle CSS code:
        Hx = [A | B],  Hz = [B^T | A^T],
    where A,B are circulant matrices built from powers of
    S = create_circulant_matrix(n, [-1]).
    """
    S = create_circulant_matrix(n, [-1])
    ...
    A_list = [S_pow(p) for p in A_pows]
    B_list = [S_pow(p) for p in B_pows]

    # Sum monomials over GF(2)
    A = np.array(reduce(lambda X, Y: X + Y, A_list)) % 2
    B = np.array(reduce(lambda X, Y: X + Y, B_list)) % 2

    hx = np.hstack((A, B))
    hz = np.hstack((B.T, A.T))

    return css_code(hx, hz, name=name, name_prefix="BICYCLE", check_css=True), A_list, B_list
